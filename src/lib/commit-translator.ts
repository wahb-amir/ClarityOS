...
const TYPE_MAP: Record<
  string,
  [string, ParsedCommit["activityType"], boolean]
> = {
...
function parseConventional(
  message: string,
): { type: string; scope?: string; subject: string } | null {
...
export function translateCommit(rawMessage: string): ParsedCommit {
  ...
  if (!parsed) {
    ...
    const activityType: ParsedCommit["activityType"] =
      /\b(fix|bug|patch|hotfix|resolve)\b/.test(lower)
        ? "BUG_FIX"
        : /\b(deploy|release|publish|ship)\b/.test(lower)
          ? "DEPLOYMENT"
          : "FEATURE_PROGRESS";
    return {
      humanText:
        cleaned.charAt(0).toUpperCase() + cleaned.slice(1) || firstLine,
      internal: false,
      activityType,
      needsReview: true,
    };
  ...
  if (internal) {
    return {
      humanText: `(internal — ${type}${scope ? `: ${scope}` : ""})`,
      internal: true,
      activityType,
      needsReview: false,
    };
  ...
  const humanText =
    `${prefix}${scopePart}${scopePart ? ": " : " "}${cleanedSubject}`
      .replace(/\s{2,}/g, " ")
      .trim();
