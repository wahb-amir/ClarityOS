...
const IntegrationStatusSchema = new Schema<IntegrationStatus>({
  status:      { type: String, enum: ['unlinked', 'linked', 'error'], default: 'unlinked' },
  webhookId:   String,
  lastEventAt: Date,
  error:       String,
}, { _id: false })
...
const ProjectSchema = new Schema<IProject>({
  ...
  vercelTeamId:         String,
  integrations: {
    github: { type: IntegrationStatusSchema, default: () => ({ status: 'unlinked' }) },
    vercel: { type: IntegrationStatusSchema, default: () => ({ status: 'unlinked' }) },
  },
}, { timestamps: true })
