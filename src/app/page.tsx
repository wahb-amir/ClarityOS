    ...
    <div className="min-h-screen bg-bg-base overflow-hidden relative">
      ...
      <section className="max-w-4xl mx-auto px-6 pt-28 pb-20 text-center relative">
        ...
        <p className="text-lg text-text-secondary leading-relaxed max-w-2xl mx-auto mb-10">
          No more &ldquo;what&apos;s the update?&rdquo; messages. ClarityOS translates your
          technical work into language clients understand — automatically.
        </p>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={f.title} className="card p-8 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group bg-white/50 backdrop-blur-md border border-white/20">
              ...
              <h3 className="font-semibold text-lg text-text-primary mb-3 tracking-tight">{f.title}</h3>
      ...
      <section className="border-t border-border bg-gradient-to-b from-bg-surface to-bg-base">
        <div className="max-w-3xl mx-auto px-6 py-24 text-center">
          <p className="text-3xl font-semibold text-text-primary tracking-tight leading-snug">
            &ldquo;If the client needs to ask for an update,<br className="hidden sm:block" />
      ...
      <footer className="border-t border-border bg-bg-surface">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-text-muted">
          <span className="flex items-center gap-2 font-medium">
            <span className="w-4 h-4 rounded bg-text-muted/20 text-text-muted flex items-center justify-center text-[10px] font-bold">C</span>
            © {new Date().getFullYear()} ClarityOS
          </span>
          <span className="text-center md:text-right">
            Built for developers who care about their clients. Made by{' '}
            <a href="https://wahb.space" target="_blank" rel="noopener noreferrer" className="text-brand font-medium hover:underline transition-all">
              Wahb Amir
            </a>
          </span>
        </div>
