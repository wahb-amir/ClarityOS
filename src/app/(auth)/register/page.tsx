...
export default function RegisterPage() {
  ...
  if (success) {
    return (
      ...
      >
        ...
        <h2 className="text-xl font-semibold text-text-primary tracking-tight mb-2">Check your email</h2>
        <p className="text-sm text-text-secondary leading-relaxed mb-1">
          We&apos;ve sent a verification link to
        </p>
        <p className="text-sm font-medium text-text-primary mb-6">{success.email}</p>
  ...
  return (
    ...
    >
      ...
      >
        {googleLoading ? (
          <svg className="animate-spin w-4 h-4 text-text-secondary" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        {/* Error */}
        {error && (
          <div role="alert" className="rounded-lg bg-danger-light border border-danger-muted px-3.5 py-3 text-sm text-danger">
            {error}
          </div>
        )}
      ...
      <p className="mt-5 text-center text-sm text-text-secondary">
        ...
        <Link href="/login" className="font-medium text-brand hover:underline underline-offset-2 transition-colors">
          Sign in
        </Link>
      </p>
    </motion.div>
