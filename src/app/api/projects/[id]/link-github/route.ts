...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  const integration = await DevIntegration.findOne({ userId: session.user.id, provider: 'github' }).lean()
  if (!integration) {
    return NextResponse.json({ error: 'GitHub not connected — connect your account first' }, { status: 400 })
  }
  ...
  if (!ghRes.ok) {
    ...
    console.log(`GitHub webhook creation failed for ${owner}/${repo} (status ${ghRes.status}):`, err)
    // 422 means hook already exists — fetch existing hooks and reuse
    if (ghRes.status !== 422) {
      return NextResponse.json({ error: `GitHub API error: ${err.message ?? ghRes.status}` }, { status: 502 })
    }
    ...
    if (existing) {
      ...
      await fetch(`https://api.github.com/repos/${owner}/${repo}/hooks/${existing.id}`, {
        ...
        body: JSON.stringify({ active: true, config: { url: webhookUrl, content_type: 'json', secret: webhookSecret } }),
      })
