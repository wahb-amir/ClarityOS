...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  if (!vercelProjectId) {
    return NextResponse.json(
      { error: "vercelProjectId is required" },
      { status: 400 },
    );
  }
  ...
  if (!integration) {
    return NextResponse.json(
      { error: "Vercel not connected — connect your account first" },
      { status: 400 },
    );
  }
  ...
  if (domain)
    project.deployUrl = domain.startsWith("http")
      ? domain
      : `https://${domain}`;
