...
export async function GET(req: NextRequest) {
  ...
  if (!integration) {
    return NextResponse.json({ error: 'GitHub not connected', connected: false }, { status: 404 })
  }

  const token = (integration as { accessToken: string }).accessToken

  // Fetch all repos (user + org) — paginate up to 3 pages (300 repos)
  const repos: Array<{ full_name: string; private: boolean; html_url: string; owner: { login: string }; name: string }> = []
...
export async function DELETE(req: NextRequest) {
  ...
  await connectDB()
  await DevIntegration.deleteOne({ userId: session.user.id, provider: 'github' })
