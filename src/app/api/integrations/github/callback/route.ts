...
export async function GET(req: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin?error=github_missing_params`)
  }
  ...
  try {
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin?error=github_bad_state`)
  }
  ...
  if (!tokenData.access_token) {
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin?error=github_token_failed`)
  }
  ...
  return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/admin?github_connected=1`)
}
