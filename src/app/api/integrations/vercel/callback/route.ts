...
export async function GET(req: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL}/admin?error=vercel_missing_params`,
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL}/admin?error=vercel_bad_state`,
    );
  }
  ...
  if (!tokenData.access_token) {
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL}/admin?error=vercel_token_failed`,
    );
  }
  ...
  return NextResponse.redirect(
    `${process.env.NEXTAUTH_URL}/admin?vercel_connected=1`,
  );
}
