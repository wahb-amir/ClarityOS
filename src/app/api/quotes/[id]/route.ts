...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  if (
    ...
    String(q.clientId?._id) !== session.user.id
  ) {
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  const { action } = body;
  const clientUser = quote.clientId as unknown as {
    _id: unknown;
    name: string;
    email: string;
  };
  ...
  if (session.user.role === "client") {
    ...
    if (action === "pause") {
      if (quote.status !== "open")
        return NextResponse.json(
          { error: "Only open quotes can be paused." },
          { status: 400 },
        );
      quote.status = "paused";
      await quote.save();
    ...
    if (action === "reopen") {
      if (quote.status !== "paused")
        return NextResponse.json(
          { error: "Only paused quotes can be reopened." },
          { status: 400 },
        );
      quote.status = "open";
      await quote.save();
    ...
    if (action === "close") {
      if (quote.status === "converted")
        return NextResponse.json(
          { error: "Converted quotes cannot be closed." },
          { status: 400 },
        );
      quote.status = "closed";
      await quote.save();
    ...
    if (action === "update") {
      ...
      if (body.text.trim().length > 2000)
        return NextResponse.json(
          { error: "Update too long (max 2000 chars)." },
          { status: 400 },
        );
      if (quote.status === "closed" || quote.status === "converted") {
        return NextResponse.json(
          { error: "Cannot update a closed or converted quote." },
          { status: 400 },
        );
      }
  ...
  if (session.user.role === "dev") {
    ...
    if (action === "convert") {
      if (quote.status === "converted") {
        return NextResponse.json(
          { error: "Quote is already converted." },
          { status: 400 },
        );
      }
      if (!body.projectName?.trim() || !body.projectDescription?.trim()) {
        return NextResponse.json(
          { error: "Project name and description are required to convert." },
          { status: 400 },
        );
      }
      ...
      quote.convertedProjectId =
        project._id as unknown as import("mongoose").Types.ObjectId;
      await quote.save();
