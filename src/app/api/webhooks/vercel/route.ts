...
type VercelDeploymentState =
  "BUILDING" | "READY" | "ERROR" | "CANCELED" | string;
...
function translateDeployment(
  ...
  switch (state) {
    case "BUILDING":
      return `Starting deployment for ${projectName}`;
    case "READY":
      return `Successfully deployed ${projectName} — live and accessible`;
    case "ERROR":
      return `Deployment for ${projectName} failed — investigating the issue`;
    case "CANCELED":
      return `Deployment for ${projectName} was cancelled`;
    default:
      return `Deployment update for ${projectName}: ${state}`;
  }
}
...
export async function POST(req: NextRequest) {
  ...
  if (!project && deployUrl) {
    ...
    project = await Project.findOne({
      deployUrl: { $regex: escaped, $options: "i" },
    }).lean();
  }

  if (!project) {
    console.warn(
      `[vercel webhook] No project matched — vercelProjectId: ${vercelProjectId}, deployUrl: ${deployUrl}`,
    );
  ...
  if (!secret) {
    console.warn(
      "[vercel webhook] No webhook secret configured — skipping signature check",
    );
  ...
  await Project.updateOne(
    ...
    {
      $set: {
        "integrations.vercel.lastEventAt": new Date(),
        "integrations.vercel.status": "linked",
      },
    },
  );
