...
export function ProjectClient({ projectId }: { projectId: string }) {
  ...
  const fetchAll = useCallback(async () => {
    const [p, a, f, b] = await Promise.all([
      ...
      fetch(`/api/projects/${projectId}/activities?published=true`).then((r) =>
        r.json(),
      ),
      ...
      {activeBlockers.length > 0 && (
        <div className="mb-6 space-y-2">
          {activeBlockers.map((b) => {
            const typeInfo = blockerTypeLabels[b.type] ?? {
              label: b.type,
              icon: "⛔",
            };
            ...
            return (
              ...
              >
                ...
                <div>
                  <p
                    className={`text-sm font-bold ${isPayment ? "text-[#7C3AED]" : "text-warning"}`}
                  >
                    {typeInfo.label}
                  </p>
                  <p className="text-text-primary text-sm mt-0.5 font-medium">
                    {b.title}
                  </p>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-bg-raised rounded-xl p-4 border border-border">
                  ...
                  <div className="flex gap-4 mt-3">
                    ...
                    <span className="text-xs text-text-muted">
                      {features.filter((f) => f.status === "review").length} in
                      review
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                {blockers.map((b) => {
                  ...
                  return (
                    <Card
                      key={b._id}
                      className={`${b.status === "resolved" ? "opacity-50" : ""} ${isPayment ? "border-l-4 border-l-[#7C3AED]" : ""}`}
                    >
