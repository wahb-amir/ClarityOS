...
const blockerStatusConfig: Record<BlockerStatus, { label: string; badgeVariant: string; color: string }> = {
  active:   { label: 'Active',   badgeVariant: 'waiting_client', color: 'text-warning' },
  pending:  { label: 'Pending',  badgeVariant: 'in_progress',    color: 'text-brand' },
...
function DevFeatureCard({
  ...
  const handleStatusChange = async (newStatus: FeatureStatus) => {
    ...
    setUpdating(true)
    try {
      const res = await fetch(`/api/projects/${projectId}/features/${feature._id}`, {
        method: 'PATCH',
  ...
  return (
    <Card className="space-y-3">
        ...
        <div className="relative flex-shrink-0">
          <select
            ...
            className={`appearance-none text-xs font-medium px-2.5 py-1 pr-6 rounded-full border cursor-pointer transition-all disabled:opacity-50 ${
              feature.status === 'done'        ? 'bg-success-light border-success-muted text-success' :
              feature.status === 'in_progress' ? 'bg-brand-light border-brand-muted text-brand' :
              feature.status === 'review'      ? 'bg-[#F5F3FF] border-[#DDD6FE] text-[#7C3AED]' :
      ...
      <p className="text-text-secondary text-xs leading-relaxed">{feature.explanation}</p>
...
function DevBlockerCard({
  ...
  const typeInfo = blockerTypeLabels[blocker.type] ?? { label: blocker.type, icon: '⛔' }

  const handleStatusChange = async (newStatus: BlockerStatus) => {
    if (newStatus === blocker.status) return
    setUpdating(true)
    try {
      const res = await fetch(`/api/projects/${projectId}/blockers/${blocker._id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: newStatus }),
      })
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-2">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <h3 className="text-text-primary text-sm font-medium truncate">{blocker.title}</h3>
        </div>
        ...
        <div className="relative flex-shrink-0">
          <select
            ...
            disabled={updating}
            onChange={e => handleStatusChange(e.target.value as BlockerStatus)}
            className={`appearance-none text-xs font-medium px-2.5 py-1 pr-6 rounded-full border cursor-pointer transition-all disabled:opacity-50 ${
              blocker.status === 'resolved' ? 'bg-success-light border-success-muted text-success' :
              blocker.status === 'pending'  ? 'bg-brand-light border-brand-muted text-brand' :
      ...
      <p className="text-text-secondary text-xs leading-relaxed mb-3">{blocker.explanation}</p>

      <div className="flex items-center gap-4 text-xs text-text-muted border-t border-border/50 pt-2">
        <span>
          Owner: <span className="text-text-secondary capitalize">{blocker.owner}</span>
        </span>
...
export function DevProjectClient({ projectId }: { projectId: string }) {
  ...
  const fetchActivities = useCallback(async () => {
    const data = await fetch(`/api/projects/${projectId}/activities?published=all`).then(r => r.json())
  ...
  const fetchFeatures = useCallback(async () => {
    const data = await fetch(`/api/projects/${projectId}/features`).then(r => r.json())
    setFeatures(Array.isArray(data) ? data : [])
  }, [projectId])

  const fetchBlockers = useCallback(async () => {
    const data = await fetch(`/api/projects/${projectId}/blockers`).then(r => r.json())
    setBlockers(Array.isArray(data) ? data : [])
  }, [projectId])

  const refreshAll = useCallback(async () => {
    await Promise.all([fetchProject(), fetchActivities(), fetchFeatures(), fetchBlockers(), fetchQueue()])
    setLoading(false)
  ...
  const handleFeatureStatusChange = (featureId: string, status: FeatureStatus) => {
    setFeatures(prev =>
      prev.map(f =>
        f._id === featureId
          ? {
  ...
  const handleBlockerStatusChange = (blockerId: string, status: BlockerStatus) => {
    setBlockers(prev => prev.map(b => (b._id === blockerId ? { ...b, status } : b)))
  ...
  const tabs: { id: Tab; label: string; icon: React.ElementType; count?: number; alert?: boolean }[] = [
    ...
    { id: 'blockers', label: 'Blockers', icon: AlertOctagon, count: activeBlockers.length },
    { id: 'review', label: 'Review', icon: Eye, count: draftCount, alert: draftCount > 0 },
    { id: 'settings', label: 'Settings', icon: Settings },
  ]

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <ProjectHeader project={project} showBackLink backHref="/admin" backLabel="All Projects" />
        ...
        {tabs.map(t => {
          ...
          return (
            ...
            >
              ...
              {tab === t.id && (
                <motion.div layoutId="dev-tab-indicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-brand rounded-full" />
              )}
            </button>
          )
        })}
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                <Card className="sticky top-20">
                  <ActivityTab projectId={projectId} onNotify={handleActivityLogged} />
                </Card>
              </div>
            </div>
          </motion.div>
        ...
        {tab === 'blockers' && (
          ...
          >
            ...
            <Card>
              <BlockerTab projectId={projectId} onNotify={handleBlockerLogged} />
            </Card>
          </motion.div>
        )}
        ...
        {tab === 'settings' && (
          <motion.div key="settings" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
            <Card>
              <SettingsTab project={project} onNotify={notify} onSuccess={handleSettingsSaved} />
            </Card>
          </motion.div>
        )}
      </AnimatePresence>
