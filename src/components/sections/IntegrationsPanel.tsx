    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="absolute z-20 mt-1.5 w-full max-h-64 overflow-auto rounded-lg border border-border bg-bg-surface shadow-lg shadow-black/20 py-1 animate-in fade-in slide-in-from-top-1 duration-100">
          {items.length === 0 ? (
            <p className="px-3 py-3 text-xs text-text-muted text-center">{emptyLabel}</p>
          ) : (
            items.map((item, idx) => {
              const id = getId(item)
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm flex items-center justify-between gap-2 transition-colors ${
                    isHighlighted ? 'bg-brand/10 text-text-primary' : 'text-text-secondary'
                  }`}
                >
...
export function IntegrationsPanel({
  ...
  // GitHub state
  const [ghStatus, setGhStatus] = useState<'idle' | 'loading' | 'connected' | 'error'>('idle')
  ...
  // Vercel state
  const [vcStatus, setVcStatus] = useState<'idle' | 'loading' | 'connected' | 'error'>('idle')
  ...
  useEffect(() => {
    ...
    fetch('/api/integrations/github/repos')
      .then(r => r.json())
      .then((d: { connected?: boolean; accountLogin?: string; repos?: GHRepo[] }) => {
        if (d.connected) {
  ...
  useEffect(() => {
    ...
    fetch('/api/integrations/vercel/projects')
      .then(r => r.json())
      .then((d: { connected?: boolean; accountSlug?: string; projects?: VCProject[] }) => {
        if (d.connected) {
  ...
  const linkGithub = async () => {
    ...
    if (!projectId) {
      console.error('IntegrationsPanel: linkGithub called with empty projectId')
  ...
  const unlinkGithub = async () => {
    if (!projectId) {
      console.error('IntegrationsPanel: unlinkGithub called with empty projectId')
      return
    }
    setGhLinking(true)
    try {
      await fetch(`/api/projects/${projectId}/link-github`, { method: 'DELETE' })
      setGhLinked(false)
      setGhSelected('')
      onSaved()
    } catch {
  ...
  const linkVercel = async () => {
    ...
    if (!projectId) {
      console.error('IntegrationsPanel: linkVercel called with empty projectId')
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/link-vercel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ vercelProjectId: proj?.id, projectName: proj?.name, domain: proj?.domain }),
      })
      if (res.ok) {
        setVcLinked(true)
        onSaved()
  ...
  const unlinkVercel = async () => {
    if (!projectId) {
      console.error('IntegrationsPanel: unlinkVercel called with empty projectId')
      return
    }
    setVcLinking(true)
    try {
      await fetch(`/api/projects/${projectId}/link-vercel`, { method: 'DELETE' })
      setVcLinked(false)
      setVcSelected('')
      onSaved()
    } catch {
  ...
  const filteredRepos = ghRepos.filter(r => r.fullName.toLowerCase().includes(ghSearch.toLowerCase()))
  const filteredProjects = vcProjects.filter(p => p.name.toLowerCase().includes(vcSearch.toLowerCase()))

  return (
    <div className="space-y-6">
      <div>
        <h3 className="font-semibold text-text-primary">Integrations</h3>
        <p className="text-xs text-text-muted mt-0.5">Connect your repo and deployment so changes stay in sync.</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-bg-surface/40 p-4 space-y-3.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-bg-raised border border-border">
              <svg className="w-4.5 h-4.5 text-text-primary" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div>
              ...
              {ghStatus === 'connected' && <StatusPill>@{ghAccount}</StatusPill>}
            </div>
          </div>
          {ghStatus === 'connected' ? (
            <button onClick={disconnectGithub} className="text-xs text-text-muted hover:text-error transition-colors">
              Disconnect
            </button>
          ) : ghStatus === 'idle' ? (
            <a
        ...
        {ghStatus === 'connected' &&
          (ghLinked ? (
            <LinkedCard label="Linked" detail={projectData.repoUrl} onUnlink={unlinkGithub} unlinking={ghLinking} />
          ) : (
              ...
              <Combobox
                ...
                renderRow={r => (
                  <span className="flex items-center gap-1.5 min-w-0">
                    {r.private && (
                      <svg className="w-3 h-3 text-text-muted shrink-0" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {vcStatus === 'connected' && <StatusPill>@{vcAccount}</StatusPill>}
            </div>
          </div>
          {vcStatus === 'connected' ? (
            <button onClick={disconnectVercel} className="text-xs text-text-muted hover:text-error transition-colors">
              Disconnect
            </button>
          ) : vcStatus === 'idle' ? (
            <a
              ...
              <Combobox
                ...
                renderRow={p => (
                  <span className="flex items-baseline gap-1.5 min-w-0">
                    ...
                    {p.domain && <span className="text-text-muted text-xs truncate">— {p.domain}</span>}
                  </span>
                )}
              />
