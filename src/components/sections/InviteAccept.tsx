...
import { CheckCircle2, AlertCircle, Loader2, LogIn, ShieldCheck } from 'lucide-react'
...
function InviteAcceptContent() {
  ...
  useEffect(() => {
    if (!token) {
      setState('error')
      setErrorMsg('No invitation token found. Please check the link you received.')
      return
    }
    ...
    <div className="min-h-screen bg-bg-base flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <span className="text-2xl font-bold text-brand tracking-tight">ClarityOS</span>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {state === 'loading' && (
            ...
            >
              ...
              <h1 className="text-text-primary font-semibold text-xl mb-2">Verifying Invitation</h1>
              <p className="text-text-secondary text-sm">Validating your invitation link and checking your account…</p>
            </motion.div>
          )}
          ...
          {state === 'success' && (
            ...
            >
              ...
              <h1 className="text-text-primary font-semibold text-xl mb-2">You&apos;re in!</h1>
              <p className="text-text-secondary text-sm mb-6">
                You&apos;ve been successfully added to the project. Redirecting you to your project dashboard…
              </p>
              {projectId && (
          ...
          {state === 'unauthenticated' && (
            ...
            >
              ...
              <h1 className="text-text-primary font-semibold text-xl mb-2">Sign in to Accept</h1>
              <p className="text-text-secondary text-sm mb-6">
                You need to be logged in to accept this invitation. Please sign in with the email address this invitation was sent to.
              </p>
          ...
          {state === 'error' && (
            ...
            >
              <div className={`w-16 h-16 rounded-full ${errorCode === 'INVITE_EMAIL_MISMATCH' ? 'bg-amber-500/15 border-amber-500/30' : 'bg-red-500/15 border-red-500/30'} border flex items-center justify-center mx-auto mb-5`}>
              ...
              <h1 className="text-text-primary font-semibold text-xl mb-2">
                {errorCode === 'INVITE_EMAIL_MISMATCH' ? 'Wrong Account' : 'Invitation Invalid'}
              </h1>
              ...
              {errorCode === 'INVITE_EMAIL_MISMATCH' && (
                <div className="bg-amber-400/10 border border-amber-400/20 rounded-xl px-4 py-3 text-left mb-6">
                  <p className="text-amber-300 text-xs leading-relaxed">
                    This invitation is bound to a specific email address for security. Please sign in with the email address that received the invitation, or ask the developer to send a new invite to your current account.
                  </p>
                </div>
              )}
              <Link href="/dashboard">
