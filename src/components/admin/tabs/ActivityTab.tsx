...
export function ActivityTab({ projectId, onNotify }: { projectId: string; onNotify: (msg: string) => void }) {
  const [loading, setLoading] = useState(false)
  const [aForm, setAForm] = useState({ type: 'FEATURE_PROGRESS', rawText: '' })

  const logActivity = async () => {
    setLoading(true)
    await fetch(`/api/projects/${projectId}/activities`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(aForm) })
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea className={inputClass} rows={3} placeholder="e.g. fixed login bug..." value={aForm.rawText} onChange={e => setAForm(f => ({ ...f, rawText: e.target.value }))} />
