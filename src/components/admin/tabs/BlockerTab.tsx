...
export function BlockerTab({ projectId, onNotify }: { projectId: string; onNotify: (msg: string) => void }) {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea 
            ...
            onChange={e => setBForm(f => ({ ...f, explanation: e.target.value }))} 
          />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select 
              className={inputClass} 
              value={bForm.type} 
              onChange={e => setBForm(f => ({ ...f, type: e.target.value }))}
            >
          ...
          <div>
            ...
            <select 
              className={inputClass} 
              value={bForm.owner} 
              onChange={e => setBForm(f => ({ ...f, owner: e.target.value }))}
            >
