      ...
      if (inviteEmail.trim().length >= 2) {
        ...
        try {
          const res = await fetch(
            `/api/users/search?q=${encodeURIComponent(inviteEmail)}`,
          );
          if (res.ok) {
  ...
  const handleInvite = async () => {
    ...
    if (res.ok) {
      ...
      if (data.alreadyInvited) {
        onNotify(
          "Client is already invited. No new email was sent, but the invite token was regenerated.",
        );
      } else if (data.emailSent) {
        onNotify("Invitation sent successfully!");
      } else {
        onNotify(
          `Invitation created successfully, but email failed to send: ${data.emailError || "Unknown error"}`,
        );
      }
      setInviteEmail("");
    } else {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditForm((f) => ({ ...f, deployUrl: e.target.value }))
                }
              />
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <p className="text-xs text-text-muted mt-1.5">
              An email will be sent to the client with a secure link to join
              this project.
            </p>
            {generatedLink && (
