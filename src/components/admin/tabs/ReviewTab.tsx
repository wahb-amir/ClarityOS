...
function ReviewItem({
  ...
  return (
    ...
    >
      <Card
        className={`${dimmed || item.internal ? "opacity-80 bg-bg-surface" : ""} border-border`}
      >
        ...
        <div className="space-y-3">
          <div className="rounded-lg bg-bg-raised border border-border px-3 py-2">
            <p className="text-2xs uppercase tracking-wider text-text-muted font-medium mb-1">
              Raw event
            </p>
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => onEditStart(item._id, item.humanText)}
                  >
...
export function ReviewTab({
  ...
  const publishAll = async () => {
    ...
    onNotify(
      `Published ${publishable.length} update${publishable.length === 1 ? "" : "s"}`,
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card padding="sm" className="bg-brand/5 border-brand/20">
          <p className="text-2xs uppercase tracking-wider text-text-muted font-medium">
            Pending
          </p>
        ...
        <Card padding="sm">
          ...
          <p className="text-2xl font-semibold text-text-muted mt-1">
      ...
      {queue.length === 0 ? (
        <Card className="text-center py-16">
          ...
          <p className="text-sm text-text-muted mt-1">
            New GitHub and Vercel events will appear here for review.
          </p>
        </Card>
      ) : (
        <div className="space-y-6">
