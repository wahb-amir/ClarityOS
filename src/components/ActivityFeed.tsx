...
const defaultConfig = { label: 'Update', color: 'text-text-secondary', bg: 'bg-bg-raised border-border', icon: '•' }
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
        ...
        {!isLast && <div className="w-px flex-1 bg-border mt-2 min-h-[1.5rem]" />}
      </div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              <span className={`inline-block text-2xs font-semibold uppercase tracking-wider ${config.color}`}>
                {config.label}
              </span>
              {showDevMeta && isDraft && (
            ...
            <p className="text-sm font-medium text-text-primary leading-snug">{activity.humanText}</p>
          </div>
          <span className="text-xs text-text-muted flex-shrink-0 mt-0.5">{timeAgo(activity.createdAt)}</span>
        </div>

        {(author || shortSha || source) && (
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            {author && <span className="text-xs text-text-muted">by {author}</span>}
            {shortSha && (
...
export function ActivityFeed({ activities, showDevMeta = false }: ActivityFeedProps) {
